...
router.get('/wizard', (req: Request, res: Response) => {
  ...
  res.json({ currentStep: wizard.current_step, stepData: JSON.parse(wizard.step_data || '{}'), prefill: { name: user?.name || '', phone: user?.phone || '', email: user?.email || '' }, categories: getAllCategories() });
});
router.put('/wizard', (req: Request, res: Response) => {
  const userId = req.user!.id; const { step, data } = req.body;
  if (!step || !data) { res.status(400).json({ error: 'Step e data são obrigatórios.' }); return; }
  let wizard = getWizardState(userId); if (!wizard) wizard = createWizardState(userId);
...
router.post('/wizard/complete', (req: Request, res: Response) => {
  ...
  try {
    const providerId = createProviderProfile(userId, { category_id: categories[0], description: description.trim(), experience_years: parseInt(experience_years) || 0, service_radius_km: parseFloat(service_radius_km) || 10, address: address.trim(), city: city.trim(), state: state.trim() });
    setProviderCategories(providerId, categories); deleteWizardState(userId);
    const profile = getProviderProfile(userId);
    res.status(201).json({ message: 'Cadastro concluído! Seu perfil já está visível nas buscas.', profile: { id: profile.id, description: profile.description, experienceYears: profile.experience_years || 0, serviceRadiusKm: profile.service_radius_km || 10, address: profile.address || '', city: profile.city, state: profile.state, active: !!profile.active, categories: getProviderCategories(providerId) } });
...
router.get('/me', (req: Request, res: Response) => {
  ...
  res.json({ id: profile.id, description: profile.description, experienceYears: profile.experience_years || 0, serviceRadiusKm: profile.service_radius_km || 10, address: profile.address || '', city: profile.city, state: profile.state, rating: profile.rating, reviewCount: profile.review_count, active: !!profile.active, categories: getProviderCategories(profile.id), createdAt: profile.created_at });
});

// Portfolio
router.post('/portfolio/upload', upload.single('photo'), async (req: Request, res: Response) => {
  const userId = req.user!.id; const tag = (req.query.tag as string) || 'Geral';
  if (!['Antes', 'Depois', 'Geral'].includes(tag)) { res.status(400).json({ error: 'Tag inválida.' }); return; }
  ...
  if (countPortfolioPhotos(profile.id) >= 10) { res.status(400).json({ error: 'Limite de 10 fotos.' }); return; }
  try {
...
router.delete('/portfolio/:photoId', (req: Request, res: Response) => {
  ...
  if (!profile) { res.status(403).json({ error: 'Sem permissão.' }); return; }
  deleteImageFile(photo.filename); deletePortfolioPhoto(req.params.photoId as string);
