...
# Categorias
cats = [
    Category(nome="Eletricista", icone="⚡", descricao="Instalações e reparos elétricos"),
    Category(nome="Pedreiro", icone="🧱", descricao="Construção e alvenaria"),
    Category(nome="Encanador", icone="🔧", descricao="Hidráulica e encanamento"),
    Category(nome="Pintor", icone="🎨", descricao="Pintura residencial e comercial"),
    Category(nome="Jardineiro", icone="🌿", descricao="Jardinagem e paisagismo"),
]
db.add_all(cats)
db.flush()
...
for data in providers_data:
    ...
    provider = Provider(
        ...
        media_avaliacao=data["nota"], total_avaliacoes=data["reviews"],
        raio_atuacao_km=20,
    )
    db.add(provider)
    db.flush()
...
db.commit()
print(f"Criados {len(providers_data)} prestadores com dados de teste")
db.close()
