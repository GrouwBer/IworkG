...
export default function ReviewSection({ providerId }: Props) {
  ...
  const loadReviews = () => {
    providerService.getReviews(providerId)
      .then(setReviews)
      .catch(() => {})
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      loadReviews();
    } catch (err: any) { setError(err.response?.data?.error || 'Erro ao enviar.'); }
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: 28, boxShadow: '0 2px 12px rgba(0,0,0,.06)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        ...
        {user && !showForm && (
          <button onClick={() => setShowForm(true)} style={s.btn}>Avaliar</button>
        )}
      </div>

      {showForm && (
        <div style={{ padding: 20, background: '#f9fafb', borderRadius: 12, marginBottom: 20, border: '1px solid #e5e7eb' }}>
          ...
          <textarea
            ...
            maxLength={300}
            style={{ width: '100%', padding: '10px 14px', fontSize: 14, border: '1px solid #d1d5db', borderRadius: 8, resize: 'vertical', minHeight: 60, marginBottom: 12, fontFamily: 'inherit', boxSizing: 'border-box' }}
          />
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleSubmit} disabled={submitting} style={s.primary}>{submitting ? 'Enviando...' : 'Enviar Avaliação'}</button>
            <button onClick={() => { setShowForm(false); setError(''); }} style={s.cancel}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      {loading ? <p style={{ color: '#9ca3af' }}>Carregando...</p> :
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {reviews.map(r => (
            <div key={r.id} style={{ padding: '16px 0', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 600, color: '#6b7280', flexShrink: 0 }}>
