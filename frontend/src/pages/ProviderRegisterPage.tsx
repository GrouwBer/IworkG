...
export default function ProviderRegisterPage() {
  ...
  useEffect(() => { if (al) return; if (!isAuthenticated) { nav('/login'); return; }
    (async () => { try {
      ...
      if (s.currentStep === 1 && f.name === '' && f.phone === '') {
  ...
  if (done) return <C m={<><span style={{ fontSize: 48 }}>🎉</span><h2>Cadastro Concluído!</h2><p style={{ color: '#16a34a' }}>Seu perfil já está ativo.</p><button onClick={() => { window.location.href = '/dashboard'; }} style={b.primary}>Ir para Dashboard</button></>} />;
...
function S1({ f, s }: { f: F; s: any }) { return <div><ST n={1} t="Dados Básicos" d="Confirme seus dados." /><label style={li}><span>Nome *</span><input style={ii} value={f.name || ''} onChange={e => s((p: F) => ({ ...p, name: e.target.value }))} /></label><label style={{ ...li, marginTop: 16 }}><span>Telefone *</span><input style={ii} type="tel" value={f.phone || ''} onChange={e => s((p: F) => ({ ...p, phone: e.target.value }))} /></label></div>; }
...
function S3({ f, s }: { f: F; s: any }) { return <div><ST n={3} t="Descrição" d="Conte sobre sua experiência." /><label style={li}><span>Descrição *</span><textarea style={{ ...ii, resize: 'vertical', minHeight: 100 }} rows={5} value={f.desc} onChange={e => s((p: F) => ({ ...p, desc: e.target.value }))} maxLength={500} /></label><span style={{ fontSize: 12, color: '#9ca3af', textAlign: 'right', display: 'block' }}>{f.desc.length}/500</span><label style={{ ...li, marginTop: 16 }}><span>Anos de experiência</span><input style={ii} type="number" value={f.exp} onChange={e => s((p: F) => ({ ...p, exp: e.target.value }))} min={0} max={60} /></label></div>; }
function S4({ f, s }: { f: F; s: any }) { return <div><ST n={4} t="Localização" d="Endereço e raio de atendimento." /><label style={li}><span>Raio (km) *</span><input style={ii} type="number" value={f.rad} onChange={e => s((p: F) => ({ ...p, rad: e.target.value }))} min={1} max={500} /></label><label style={{ ...li, marginTop: 16 }}><span>Endereço *</span><input style={ii} value={f.addr} onChange={e => s((p: F) => ({ ...p, addr: e.target.value }))} /></label><div style={{ display: 'flex', gap: 12, marginTop: 16 }}><label style={{ ...li, flex: 1 }}><span>Cidade *</span><input style={ii} value={f.city} onChange={e => s((p: F) => ({ ...p, city: e.target.value }))} /></label><label style={{ ...li, flex: 1 }}><span>Estado *</span><input style={ii} value={f.state} onChange={e => s((p: F) => ({ ...p, state: e.target.value }))} maxLength={2} /></label></div></div>; }
